...
def densityI(seqpath, reps):
    ...
    for i in range(reps):
        cmd = 'python3 decsymodF.py ' + str(k) + ' ' + str(L) + ' ' + seqpath
...
def compare(akmer, bkmer, coeffs):
    # print(f'akmer is {akmer} and bkmer is {bkmer}')
    membership = [(functions.checkIndecmod(akmer, coeffs), functions.checkInsymmod(akmer, coeffs)),
                  (functions.checkIndecmod(bkmer, coeffs), functions.checkInsymmod(bkmer, coeffs))]
