...
def densityI(seqpath, reps):
    ...
    for i in range(reps):
        cmd = 'python3 decsymF.py ' + str(k) + ' ' + str(L) + ' ' + seqpath
...
def compare(akmer, bkmer, coeffs):
    ...
    membership = [(functions.checkIndec(akmer, coeffs), functions.checkInsym(akmer, coeffs)),
                  (functions.checkIndec(bkmer, coeffs), functions.checkInsym(bkmer, coeffs))]
